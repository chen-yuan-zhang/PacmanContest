...
class attackAgent(CaptureAgent):
    ...
    def registerInitialState(self, gameState):  # 15s
        ...
        self.foodNum = len(self.getFoodYouAreDefending(gameState).asList())
        ...
        self.keep = False
        self.setState()
        self.lastFoodDefend = self.getFoodYouAreDefending(gameState).asList()
    ...
    def updateAgentState(self, gameState):
        ...
        if len(self.getFoodYouAreDefending(gameState).asList()) > len(self.lastFoodDefend):
            self.lastFood = None
            self.keep = False

        foodEaten = list(set(self.lastFoodDefend) - set(self.getFoodYouAreDefending(gameState).asList()))
        self.lastFoodDefend = self.getFoodYouAreDefending(gameState).asList()
        if len(foodEaten) > 0:
            self.lastFood = foodEaten[0]
            self.foodTime = 0
        else:
            if not self.keep:
                self.foodTime += 1
        ...
        if self.invaders:
            self.lastFood = self.invaders[0].getPosition()
            self.foodTime = 0
        ...
        if not self.attack and self.getScore(gameState)<0:
            self.attack = True
            global defend
            defend = False

        global defend
        if not defend and len(self.getFoodYouAreDefending(gameState).asList()) + 2 < len(
                self.getFood(gameState).asList()) and len(
            self.getFoodYouAreDefending(gameState).asList()) * 2 < self.foodNum:
            self.attack = False
            defend = True
    ...
    def getInvaderDistance(self, gameState):
        if self.invaders:
            invaderDistance = [self.getMazeDistance(a.getPosition(), gameState.getPosition()) for a in self.invaders]
            return max(min(invaderDistance), gameState.scaredTimer + 1)
        else:
            return 9999
    ...
    def chooseAction(self, gameState):
        ...
        if self.getInvaderDistance(self.myState) == 1:
        ...
        if self.attack:
            ...
            action = self.getDefendActionNew(gameState)
    ...
    def findGhosts(self):
        ...
        if len(self.ghostsDistanceHistory) >= 5 and 9999 > self.ghostsDistanceHistory[-1] == self.ghostsDistanceHistory[
            -2] == self.ghostsDistanceHistory[-3] == self.ghostsDistanceHistory[-4] == self.ghostsDistanceHistory[
            -5] and self.myState.numCarrying > 0:
            return True

        if self.ghostsDistanceHistory[-1] < 2:
            return True
    ...
    def getAttackAction(self, gameState):
        ...
        if enemyFoodLeft <= 2:
            if self.isPacman:  # go back home
                return self.escapeAction
            else:
                return self.getDefendActionNew(gameState)
    ...
    def getDefendActionNew(self, gameState):
        ...
        range = 0
        if self.lastFood:
            for position in deadRoad:
                if (self.lastFood in deadRoad[position]) and len([food for food in deadRoad[position] if
                                                                  food in self.getCapsulesYouAreDefending(
                                                                      gameState)]) == 0:
                    if len(deadRoad[position]) > range:
                        keyPosition = position
                        range = len(deadRoad[position])

            if keyPosition:
                if self.getMazeDistance(keyPosition, self.lastFood) - self.foodTime >= self.getMazeDistance(keyPosition,
                                                                                                            self.position) - 1:
                    legalActions = gameState.getLegalActions(self.index)
                    distance = []
                    for action in legalActions:
                        successor = self.getSuccessor(gameState, action)
                        myState = successor.getAgentState(self.index)
                        distance.append(self.getMazeDistance(myState.getPosition(), keyPosition))

                    minD = min(distance)
                    if minD == 0:
                        self.keep = True
                    bestActions = [a for a, v in zip(legalActions, distance) if v == minD]
                    return random.choice(bestActions)

        foodNum = 0
        for position in deadRoad:
            if keyPosition in deadRoad:
                if (len([food for food in self.getFoodYouAreDefending(gameState).asList() if
                         food in deadRoad[position]]) > foodNum or ((
                                                                            len([food for food in
                                                                                 self.getFoodYouAreDefending(
                                                                                     gameState).asList() if
                                                                                 food in deadRoad[
                                                                                     position]])) == foodNum and len(
                    deadRoad[position]) > len(
                    deadRoad[keyPosition]))) and (len(
                    [food for food in self.getCapsulesYouAreDefending(gameState) if
                     food in deadRoad[position]]) == 0 or not self.lastFood in deadRoad[position]):
                    keyPosition = position
                    foodNum = len([food for food in self.getFoodYouAreDefending(gameState).asList() if
                                   food in deadRoad[position]])
            else:
                if len([food for food in self.getFoodYouAreDefending(gameState).asList() if
                         food in deadRoad[position]]) > foodNum  and (len(
                    [food for food in self.getCapsulesYouAreDefending(gameState) if
                     food in deadRoad[position]]) == 0 or not self.lastFood in deadRoad[position]):
                    keyPosition = position
                    foodNum = len([food for food in self.getFoodYouAreDefending(gameState).asList() if
        ...
        if keyPosition and foodNum >= len(self.getFood(gameState).asList()):
            legalActions = gameState.getLegalActions(self.index)
            distance = []
            for action in legalActions:
                successor = self.getSuccessor(gameState, action)
    ...
    def getDefendAction(self, gameState):
        # print self.getInvaderDistance(self.myState)
        # raw_input("Enter to continue...")
        if 0 < self.getInvaderDistance(self.myState) < 5:
            legalActions = gameState.getLegalActions(self.index)
            distance = []
            for action in legalActions:
                successor = self.getSuccessor(gameState, action)
                myState = successor.getAgentState(self.index)
                distance.append(self.getInvaderDistance(myState))

            minD = min(distance)
            bestActions = [a for a, v in zip(legalActions, distance) if v == minD]
            return random.choice(bestActions)
        else:
            return self.protectFood(gameState)

    def protectFood(self, gameState):

        nearestFood = None
        nearestDistance = 9999
        if self.lastFood:
            position = self.lastFood
        else:
            position = self.position

        for food in self.getFoodYouAreDefending(gameState).asList() + self.getCapsulesYouAreDefending(gameState):
            thisD = self.getMazeDistance(food, position)
            if thisD < nearestDistance:
                nearestFood = food
        ...
        for action in legalActions:
            ...
            distance.append(self.getMazeDistance(nextPos, nearestFood))

        minD = min(distance)
        bestActions = [a for a, v in zip(legalActions, distance) if v == minD]
        return random.choice(bestActions)

    def getDefendActionOld(self, gameState):
