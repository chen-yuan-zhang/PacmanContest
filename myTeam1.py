...
class attackAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if self.getPreviousObservation():
            ...
            if not friendState.isPacman and friendState.getPosition()==friendlastState.getPosition():
                self.attack = True
    ...
    def getAttackAction(self,gameState):
        ...
        values = [self.evaluate(gameState, a) for a in legalActions]
        maxValue = max(values)
        bestActions = [a for a, v in zip(legalActions, values) if v == maxValue]

        return random.choice(bestActions)# usual move
    ...
    def escape(self, gameState):
      ...
      for a in self.ghosts:
        x,y = a.getPosition()
        dir = a.getDirection()
        x1 = Actions.directionToVector(dir)[0] + x
        y1 = Actions.directionToVector(dir)[1] + y
        #print x1, y1
        walls[int(x)][int(y)]=True
        walls[int(x1)][int(y1)] = True
      ...
      while not openList.isEmpty():
        ...
        if currentPosition not in closeList:
          if self.isSafePosition(currentPosition, gameState):
            ...
            self.safeDistance = currentNode[3]
            if (len(path)==0):
    ...
    def findFood(self, gameState):
      ...
      for a in self.ghosts:
        ...
        walls[int(x)][int(y)]=True
        walls[int(x1)][int(y1)] = True
        if y1+Actions.directionToVector(dir)[1]<self.mapHeight:
            walls[int(x1)][int(y1+Actions.directionToVector(dir)[1])] = True
        walls[int(x)][int(y + 1)] = True
        walls[int(x)][int(y - 1)] = True
    ...
    def findPossiblePaths(self, gameState):  # return nothing
        ...
        while (y1 <= yMax):
            ...
            while walls[self.safeX][y1]: y1 += 1

            y2 = y1

            while not walls[self.safeX][y2]: y2 += 1
    ...
    def getDWeights(self, gameState, action):
        ...
        return {'onDefense': 100000,  # never across midline when defending

                'numInvaders': -5000, 'invaderDistance': -1000, 'distanceToSuperPacman': 2000,
                # when there is an invader these features's weights are dominent

                'destanceToDoor': -6,
                'distanceToMidNode': -1, }  # when there is no invader thses weights will control our defending agent
